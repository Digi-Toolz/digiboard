...
function renderWall(c){
  ...
  stage.innerHTML=`
    ...
    <div class="tree-scene-v22 growth-${Math.min(4,Math.floor(state.leafCount/100))}" aria-label="Klassenbaum mit ${heroCount} Waldheldinnen und Waldhelden">
      ...
      <div class="forest-story-glints-v1573" aria-hidden="true"></div>
      <div class="forest-story-layer-v1573" aria-hidden="true">
        <span class="forest-story-owl-v1573">🦉</span>
        <span class="forest-story-bird-v1573">🐦</span>
        <span class="forest-story-acorn-v1573">🌰</span>
        <span class="forest-story-ladybird-v1573">🐞</span>
      </div>
...
async function exportDigiBoardBackupFile(){
  ...
  const payload={format:'digiboard-backup',version:2,createdAt:new Date().toISOString(),appVersion:'15.77',state:exportState},json=JSON.stringify(payload,null,2),fileName=`DigiBoard-${state.classWorld?.className||'Klasse'}-${date}.digiboard-backup.json`,file=new File([json],fileName,{type:'application/json'});
  try{
...
async function exportPersonalProfileFile(){
  ...
  const payload={format:'digiboard-personal-profile',version:1,createdAt:new Date().toISOString(),appVersion:'15.77',person:{sourceId:member.id,name:member.profilePrefs?.displayName||member.name,role:member.role,profilePrefs:clone(member.profilePrefs||{}),teachingTools:clone(member.teachingTools||[])},materials:clone(state.materials?.[member.id]||{activeDrawer:'Allgemein',drawers:[]})};
